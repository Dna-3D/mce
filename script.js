...
function initHallOfFameTabs() {
    const tabs = Array.from(document.querySelectorAll('.hall-tabs .tab'));
    ...
    tabs.forEach(t => t.addEventListener('click', () => {
        ...
        tabs.forEach(tb => {
            tb.classList.toggle('active', tb === t);
            tb.setAttribute('aria-selected', tb === t ? 'true' : 'false');
        });

        // show/hide panels
        const panels = document.querySelectorAll('.hall-grid[role="tabpanel"]');
        panels.forEach(p => {
            const id = p.id;
            if (id === target) {
                p.hidden = false;
